...
func (c *Conn) makeClientHello() (*ClientHelloMsg, ecdheParameters, error) {
	...
	hello := &ClientHelloMsg{
		vers:                         config.ClientHelloVersion,
		compressionMethods:           config.CompressionMethods,
		random:                       make([]byte, 32),
		sessionId:                    make([]byte, 32),
		ocspStapling:                 config.OscpStapling,
		scts:                         config.Scts,
		serverName:                   hostnameInSNI(config.ServerName),
		supportedCurves:              config.SupportedCurves,
		supportedPoints:              config.SupportedPoints,
		secureRenegotiationSupported: config.SecureRenegotiationSupported,
		alpnProtocols:                config.NextProtos,
		supportedVersions:            config.SupportedVersions,
		supportedSignatureAlgorithms: config.SupportedSignatureAlgorithms,
		ticketSupported: config.TicketSupported,
		pskModes: config.PskModes,
		extensionOrder: config.Extensions,
		raw: config.RawClientHello,
	}
	...
	hello.cipherSuites = config.CipherSuites
